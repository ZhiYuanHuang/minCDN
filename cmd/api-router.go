...
func newMemoryCacheFn() *freecache.Cache {
	globalObjLayerMutex.RLock()
	defer globalObjLayerMutex.RUnlock()
	return globalMemoryCache
}

func setMemoryCache(cache *freecache.Cache) {
	globalObjLayerMutex.Lock()
...
func registerAPIRouter(router *mux.Router) {
	...
	for _, router := range routers {
		...
		router.Methods(http.MethodGet).Path("/stranger/{object:.+}").HandlerFunc(gz(httpTraceHdrs(api.GetImageHandle)))
	}
